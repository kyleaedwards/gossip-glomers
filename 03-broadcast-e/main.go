...
func createRPCPool(n *maelstrom.Node, poolSize int) chan BroadcastMessagePayload {
	...
	for i := 0; i < poolSize; i += 1 {
		go func() {
			for {
				msg := <-ch
				for {
					err := n.Send(msg.dest, msg.broadcastMessage)
					if err == nil {
						msg.wg.Done()
						break
					}
